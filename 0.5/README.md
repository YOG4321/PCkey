...
从 `0.4.2-dev` 开始，Core 和 Editor 以“可响应健康检查的窗口”判断健康
实例。终端安全软件留下的无窗口残留进程不会再阻止新实例启动。建议始终
普通双击启动，不要使用“以管理员身份运行”。

从 `0.4.3-dev` 开始，可以右键任务栏通知区域中的紫色 `P` 图标并选择
“退出”。退出时会先关闭编辑器、停止键盘钩子、释放 PCkey 持有的合成
按键并删除托盘图标，然后正常结束 Core 进程；不会使用容易留下安全软件
残留线程的强制终止方式。

从 `0.4.4-dev` 开始，顶部提供“按键测试”按钮。物理模式显示键盘硬件
上报的扫描码与按键名称；配置模式显示当前已经“保存并应用”的配置实际
输出，包括普通键、快捷操作、媒体键和鼠标动作。测试事件只在页面打开时
通过内存消息显示，不写入日志或配置文件。测试期间键盘输出会被拦截，
快捷键、媒体键和鼠标动作只显示而不会实际执行。
...
v0.4 已完成 Tap Dance、Combo、按键宏、Key Override、媒体键、鼠标键和
